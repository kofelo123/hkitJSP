			...
			<%
				...
				try {
					for (int i = 0; i < a.length; i++) {
						id=Long.parseLong(a[i].trim()); // id를 a[i].trim()하는것으로 가져올수 있는건지? a=session.getValueNames();
					...
					out.println("[<A href=\"order_info.jsp?total=" + total + "&count=" + count + "&quantity="+qty+"&id="+id+"\">주문하기</A>]");
				} catch (SQLException e) {
					out.println(e);
				}
			%>
